...
```
{
  "kms_key_alias": "meta-activations",
  ...
  "glue_target_table_name": "meta_activation_audience",
  "glue_catalog_target_db_name": "meta-activation",
  "glue_catalog_target_table_name": "meta_activation_audience",
...
AWS Glue DataBrew service can be used as an alternative to the glue job that generates transformed data needed for Meta activation. Use the [sample Glue DataBrew recipe](/assets/databrew/octank-collab-meta-activation-prep-recipe.json)  available in the repo as a starting point to setup a AWS Glue DataBrew Job that generates output files which inturn triggers the lambda function for sending data to Meta Business API. 
