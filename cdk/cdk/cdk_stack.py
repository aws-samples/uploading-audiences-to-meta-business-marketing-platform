...
class CdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.set_kms_key()
        self.role_name = "cleanroom_meta_activation_role"
        ...
        self.parameter_prefix = "dev/cleanroom-activations/meta"
    ...
    def update_bucket_policy(self, bucket: s3.Bucket) -> None:
        ...
        bucket_policy_statement = iam.PolicyStatement(
            sid="metaActivationsBucketPolicy",
            actions=[
                "s3:PutObject",
                "s3:GetObject",
                "s3:ListBucket",
    ...
    def add_iam_role(self) -> None:
        ...
        self.loggroup_policy_statement = iam.PolicyStatement(
            sid="metaActivationsLogGroupAccess",
            actions=[
                "logs:AssociateKmsKey"
            ],
            resources=[self.glue_log_group_arn],
        )
    ...
    def get_s3_bucket(self, bucket_name:str) -> s3.Bucket:
        ...
        s3_bucket = s3.Bucket(
            ...
            # TODO Customer managed key deployment fails, woraround is to use AWS managed Key
    ...
    def add_glue_jobs(self) -> None:
        ...
        arguments = {
            ...
            "--sourcebucket":	self.glue_source_bucket_name,
            "--targetbucket": self.glue_target_bucket_name,
    ...
    def add_lambda_layers(self) -> None:
        ...
        """
        # TODO layer classes are not getting detected when deployed through CDK, workaround is to manually create layers in console
        self.fb_lambda_layer = _lambda.LayerVersion(self, "FacebookBusiness14.0.0-Python39",
            removal_policy=RemovalPolicy.RETAIN,
            code=_lambda.Code.from_asset(path.join(self.asset_dir, "lambda/layers/1/facebook_business.zip")),
            # As part of s3 deployment, these assets are already uploaded to s3
            # TODO referencing s3 instead of local asset
            # code=_lambda.Code.from_bucket(self.cdk_asset_bucket, "lambda/layers/1/facebook_business.zip",),
    ...
    def add_event_framework(self) -> None:
        ...
        rule = events.Rule(self, "meta_activation_s3_object_create",
